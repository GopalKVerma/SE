	...
	$.fn.simpletreeview = function(options) {
		...
		function toggle($ul, method, callback) {
			...
			// Set callback to empty function if undefined
                        if( $ul.parent("li").hasClass("expandable") ){
                            $ul.parent("li").addClass("expanded");
                            $ul.parent("li").removeClass("expandable");
                        }
                        else if( $ul.parent("li").hasClass("expanded") ){
                            $ul.parent("li").addClass("expandable");
                            $ul.parent("li").removeClass("expanded");
                        }
                        if(settings.slide) { 
                              $ul.slideToggle(settings.speed, callback); 
                        }
		        else { 
                           $ul.toggle(); 
                           callback(); 
